...
void BinaryTree<T>::reflect(BNode<T> *node)
{
    if (node) {
        ...
        node->setLeft(nodo->getRight());
        node->setRight(aux);
        reflect(nodo->getLeft());
        reflect(nodo->getRight());
    }
}
template <class T>
...
bool BinaryTree<T>::sonHermanos(BNode<T> * n1, BNode<T> * n2) {
    if (n1 == NULL || n2 == NULL)
        return false;
    return (getParent(n1->getInfo()) == n2->getParent(n2->getInfo()));
}
